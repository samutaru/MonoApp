...
public AuthResponse register(UserRegisterDto reg) {
var u = new User();
u.setName(reg.getName()); u.setMail(reg.getMail());
u.setPassword(encoder.encode(reg.getPassword()));
u.setCigInitial(reg.getCigInitial()); u.setCigPrice(reg.getCigPrice());
u = userRepo.save(u);
